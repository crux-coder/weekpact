...
const [actor,member,outsider,crew,pact,s1,s2,s3]=[1,2,3,10,20,101,102,103].map(id);
...
await db.query("insert into crew_pacts(id,crew_id,title,frequency,days_per_week,created_by) values($1,$2,'Read','daily',7,$3)",[pact,crew,actor]);
...
await db.exec('delete from private.notification_events');await save([]);await save([pact]);
