...
const [owner, member, other, crew, goal, otherGoal, session] = [1,2,3,10,20,21,30].map(id);
await db.exec(`
...
for (const [g,creator] of [[goal,owner],[otherGoal,other]]) await db.query("insert into crew_goals(id,crew_id,title,frequency,days_per_week,created_by) values($1,$2,'Read','weekly',3,$3)",[g,crew,creator]);
for (const [g,user] of [[goal,owner],[goal,member],[otherGoal,owner],[otherGoal,other]]) await db.query('insert into goal_check_ins(goal_id,user_id,completed_on) values($1,$2,current_date)',[g,user]);
...
await scenario(async()=>{
 ...
 assert.equal(await count('select * from crew_goals where id=$1',[goal]),0);
 assert.equal(await count('select * from goal_check_ins where goal_id=$1',[goal]),0);
 assert.equal(await count('select * from goal_check_ins where user_id=$1',[owner]),0);
 assert.equal(await count('select * from goal_check_ins where goal_id=$1 and user_id=$2',[otherGoal,other]),1);
 ...
 assert.equal(await count("select * from private.notification_events where payload->>'goal_id'=$1",[goal]),0);
});
await scenario(async()=>{
 await db.query('delete from auth.users where id in ($1,$2)',[member,other]);
 await db.query('delete from auth.users where id=$1',[owner]);
 assert.equal(await count('select * from crews'),0);
 assert.equal(await count('select * from crew_goals'),0);
});
await scenario(async()=>{
 ...
 assert.equal(await count('select * from goal_check_ins where user_id=$1',[member]),0);
 assert.equal(await count('select * from crew_goals'),2);
});
await scenario(async()=>{
